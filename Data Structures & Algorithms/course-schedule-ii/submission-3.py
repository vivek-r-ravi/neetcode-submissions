...
# alternate way: can use indegree=0 to find starting courses instead of DFS from every course
class SolutionDFS:
    def findOrder(self, numCourses: int, prerequisites: List[List[int]]) -> List[int]:
        ...
        for i in range(numCourses): # can use indegree=0 to find starting courses instead of starting from every course
            if not dfs(i):
                return []
