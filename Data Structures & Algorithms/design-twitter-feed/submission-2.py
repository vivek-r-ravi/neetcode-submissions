...
# O(1) time for init, follow, unfollow, postTweet; O(nlogn) time for newsfeed
# O(N*m + N*M) space; add'l O(n) space for newsfeed
class Twitter:
    def __init__(self):
    ...
    def getNewsFeed(self, userId: int) -> List[int]:
        ...
        for user in self.users_followers[userId]:
            if self.users_tweets[user]:
                heapq.heappush_max(
                    max_heap,
                    (
                        self.users_tweets[user][-1][0],
                        user,
                        len(self.users_tweets[user]) - 1,
                        self.users_tweets[user][-1][1],
                    ),
                )
        while len(out) < 10 and max_heap:
            ...
            if idx >= 0:
                heapq.heappush_max(
                    max_heap,
                    (self.users_tweets[user][idx][0], user, idx, self.users_tweets[user][idx][1]),
                )
        return out
