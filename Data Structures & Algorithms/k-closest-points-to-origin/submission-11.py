...
# note: repeated computation of distance can be avoided by storing computed distances
# space complexity will increase to O(n) in this case
'''
class Solution:
    def kClosest(self, points: List[List[int]], k: int) -> List[List[int]]:
        for pt in points:
    ...
    def quickSelect(self, points: List[List[int]]:, k:int, left:int, right:int) -> List[List[int]]:
